...
describe('AppHeader', () => {
  beforeEach(() => {
    vi.mocked(usePathname).mockReturnValue('/app/courses');
  });

  it('shows the brand on the courses list page', () => {
    render(<AppHeader actions={<button type="button">Generate</button>} />);

    expect(screen.getByRole('link', { name: /build my course/i })).toHaveAttribute(
      'href',
      '/app/courses'
    );
  ...
  it('shows a back link on a course page', () => {
    vi.mocked(usePathname).mockReturnValue('/app/courses/course-1');
  ...
  it('shows a lesson back link when inside a lesson', () => {
    vi.mocked(usePathname).mockReturnValue('/app/courses/course-1/lessons/lesson-1');
